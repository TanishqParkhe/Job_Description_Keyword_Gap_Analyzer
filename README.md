...
1. **Individual Analysis** — compare one resume with one job description, inspect evidence, and download a readable PDF report.
2. **HR Screening** — upload many resumes or one ZIP archive, apply a configurable Job Match cutoff, and download shortlisted resumes.

The normal analysis is fast and does not require a cloud API. OCR and the local AI coach are optional enhancements.

## First-time setup on Windows

1. Extract the ZIP to a normal folder, preferably outside OneDrive.
2. Double-click **`setup_windows.bat`**.
3. Allow it to create `.venv` and install the Python packages.
4. Choose whether to install the optional local AI coach.
5. After setup completes, double-click **`run_app.bat`**.
